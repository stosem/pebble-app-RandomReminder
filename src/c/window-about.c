...
void window_about_init(void) {
  ...
  bmp_layer = bitmap_layer_create( GRect(PBL_IF_ROUND_ELSE(78 ,2), PBL_IF_ROUND_ELSE(2,2), 25, 25) );
  if ( bmp ) {
  gbitmap_destroy( bmp );
  bmp = NULL;
  };
  ...
  s_label_layer = text_layer_create( GRect( 6, bounds.origin.y+PBL_IF_ROUND_ELSE(24,4), bounds.size.w-12, bounds.size.h));
  ...
  text_layer_set_text( s_label_layer, "RandomTimer.\n\
Free and OpenSource.\n\
github.com/stosem/pebble-app-RandomTimer\n\
